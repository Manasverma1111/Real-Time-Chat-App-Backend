...
class AuthResourceTest {
    ...
    void changePassword_shouldReturnSuccessMessage() throws Exception {
        ...
        Mockito.when(customUserDetails.getUsername())
                .thenReturn("test@mail.com");

        Mockito.when(customUserDetails.getAuthorities())
                .thenReturn(List.of());
        ...
        // ✅ FIX: Set auth in SecurityContext so @AuthenticationPrincipal resolves correctly
        ...
        try {
            ...
            SecurityContextHolder.clearContext(); // ✅ Always clean up so other tests aren't affected
        }
    }
